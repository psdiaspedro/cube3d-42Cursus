...
/*   Updated: 2022/04/11 13:09:37 by paugusto         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_background(t_game *game)
{
	...
	while (col < WIDTH)
	{
		...
		{
			if (row < HEIGHT / 2)
				my_mlx_pixel_put(&game->canvas, (t_vec){.x = col, .y = row}, 211211211);
			else
				my_mlx_pixel_put(&game->canvas, (t_vec){.x = col, .y = row}, 210211211);
			row++;
		}
		col++;
	}
